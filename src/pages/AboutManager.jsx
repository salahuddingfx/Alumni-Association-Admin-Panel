...
const AboutManager = () => {
  ...
  const fetchSettings = async () => {
    try {
      const timelineRes = await api.get(`${API_URL}/api/v1/settings/timeline_events`);
      ...
      const advisorRes = await api.get(`${API_URL}/api/v1/settings/advisor_messages`);
  ...
  const handleAddTimelineEvent = async (e) => {
    ...
    try {
      ...
      const res = await api.put(`${API_URL}/api/v1/settings/timeline_events`, {
  ...
  const handleSaveAdvisors = async (e) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const res = await api.put(`${API_URL}/api/v1/settings/advisor_messages`, {
        value: { advisors }
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
